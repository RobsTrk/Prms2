...
async function fetchPatients() {
    try {
        ...
        console.error(err);
        document.getElementById('patient-list').innerHTML = `<p class="text-red-500 p-4 text-xs">🔴 Backend Offline. Ensure 'main.py' is running.</p>`;
    }
}
...
function selectPatient(id) {
    ...
    document.getElementById('p-id').innerText = currentPatient.id;
    document.getElementById('p-gender').innerText = currentPatient.gender;
    document.getElementById('p-dob').innerText = currentPatient.dob;

    // Status Badge
    const badge = document.getElementById('p-status');
    badge.innerText = currentPatient.status;
    ...
    // Scans
    const scanRecords = (currentPatient.history || []).filter(h => h.type === 'Radiology' || h.type.includes('Scan'));
...
async function updatePatientCondition() {
    ...
    // 1. Save Note/History
    if (note || status) {
        await fetch(`${API_BASE}/patients/${currentPatient.id}/record`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    if (Object.keys(updates).length > 0) {
        await fetch(`${API_BASE}/patients/${currentPatient.id}/update`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function addVital() {
    const val = parseInt(document.getElementById('vitals-input').value);
    ...
    document.getElementById('vitals-input').value = "";
    fetchPatients();
}
...
async function addInstruction() {
    const text = document.getElementById('new-order-text').value;
    ...
    document.getElementById('new-order-text').value = "";
    fetchPatients();
}
...
async function addScan() {
    ...
    if (!date || !currentPatient) return alert("Select Date");
...
async function toggleEmergency() {
    await fetch(`${API_BASE}/emergency?active=${!isEmergency}`, { method: 'POST' });
}
